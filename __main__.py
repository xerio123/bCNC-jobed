...
class Application(Toplevel,Sender):
	def __init__(self, master, **kw):
		...
		for name,page in self.pages.items():
			...
			for n in Utils.getStr(Utils.__prg__,"%s.page"%(page.name)).split():
				...
				try:
					...
					if last == "*":
						page.addPageFrame(n[:-1],side=placement, fill=_fill,expand=_expand,padx=padx,ipadx=0,pady=_ipady)
					else:
						page.addPageFrame(n,side=placement, fill=_fill,expand=_expand,padx=padx,ipadx=0,pady=_ipady)
		...
		self.job	  = Page.frames["RunFrame"]
		self.MainCanvas = Page.frames["MainCanvas"]
		self.editor   = Page.frames["Editor"].editor
		self.terminal = Page.frames["Terminal"].terminal
		self.buffer   = Page.frames["Terminal"].buffer
		self.jobdro	  = Page.frames["JobDRO"]
		self.jobcontrol	  = Page.frames["JobControl"]

		self.job.bind(self.gstate)
		self.jobdro.bind(self.gstate)
		#now we can do it
		self.gstate.overrideCombo.set(OVERRIDES[0])

		# XXX FIXME Do we need it or I can takes from Page every time?
		self.autolevel = Page.frames["Probe:Autolevel"]
		
		#bad use of class dependancy
		self.MainCanvas.update_editor(self.editor)

		# Command bar
		self.tt = Page.frames["Terminal"]
		f = Frame(self.tt,pady=0)
		f.grid(row=2, column=0,columnspan=2, sticky=NSEW)
		self.cmdlabel = Button(f, 
							text=_("Command:"),
							font = ("Helvetica",13),
							command=self.cmdExecute,
							)
		self.cmdlabel.pack(side=LEFT,pady=0)
		self.command = Entry(f, 
							relief=SUNKEN,
							font = ("Helvetica",12),
							background="White")
		self.command.pack(side=RIGHT, fill=BOTH, expand=YES)
		...
		self.tt.grid_rowconfigure(3, weight=1)
		...
		if self._swapKeyboard == 1:
			self.bind('<Right>',		self.jobcontrol.moveYup)
			self.bind('<Left>',		self.jobcontrol.moveYdown)
			self.bind('<Up>',		self.conjobcontroltrol.moveXdown)
			self.bind('<Down>',		self.jobcontrol.moveXup)
		elif self._swapKeyboard == -1:
			self.bind('<Right>',		self.jobcontrol.moveYdown)
			self.bind('<Left>',		self.jobcontrol.moveYup)
			self.bind('<Up>',		self.jobcontrol.moveXup)
			self.bind('<Down>',		self.jobcontrol.moveXdown)
		else:
			self.bind('<Right>',		self.jobcontrol.moveXup)
			self.bind('<Left>',		self.jobcontrol.moveXdown)
			self.bind('<Up>',		self.jobcontrol.moveYup)
			self.bind('<Down>',		self.jobcontrol.moveYdown)
		...
		try:
			...
			if self._swapKeyboard==1:
				self.bind('<KP_Right>',	self.jobcontrol.moveYup)
				self.bind('<KP_Left>',	self.jobcontrol.moveYdown)
				self.bind('<KP_Up>',	self.jobcontrol.moveXdown)
				self.bind('<KP_Down>',	self.jobcontrol.moveXup)
			elif self._swapKeyboard==-1:
				self.bind('<KP_Right>',	self.jobcontrol.moveYdown)
				self.bind('<KP_Left>',	self.jobcontrol.moveYup)
				self.bind('<KP_Up>',	self.jobcontrol.moveXup)
				self.bind('<KP_Down>',	self.jobcontrol.moveXdown)
			else:
				self.bind('<KP_Right>',	self.jobcontrol.moveXup)
				self.bind('<KP_Left>',	self.jobcontrol.moveXdown)
				self.bind('<KP_Up>',	self.jobcontrol.moveYup)
				self.bind('<KP_Down>',	self.jobcontrol.moveYdown)
		except TclError:
			pass

		self.bind('<Key-plus>',		self.jobcontrol.incStep)
		self.bind('<Key-equal>',	self.jobcontrol.incStep)
		self.bind('<KP_Add>',		self.jobcontrol.incStep)
		self.bind('<Key-minus>',	self.jobcontrol.decStep)
		self.bind('<Key-underscore>',	self.jobcontrol.decStep)
		self.bind('<KP_Subtract>',	self.jobcontrol.decStep)

		self.bind('<Key-asterisk>',	self.jobcontrol.mulStep)
		self.bind('<KP_Multiply>',	self.jobcontrol.mulStep)
		self.bind('<Key-slash>',	self.jobcontrol.divStep)
		self.bind('<KP_Divide>',	self.jobcontrol.divStep)

		self.bind('<Key-1>',		self.jobcontrol.setStep1)
	...
	def _monitorSerial(self):
		...
		#NEED TO UNLOCK AT START FOR GBRL 0.9
		...
		if self._posUpdate:
			...
			self.job.updateSpeed()
			self.dro.updateState()
			self.jobdro.updateState()
			self.jobdro.updateCoords()
